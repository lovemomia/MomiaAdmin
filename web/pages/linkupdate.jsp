...
<body>
				...
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-edit"></i> 修改商品链接</h2>
						<div class="box-icon">
							<a href="${ctx}/goodslink/info.do?uid=${user.id}&gid=${gid}" class="btn btn-back btn-round"><i class="icon-remove"></i></a>
						</div>
					</div>
					<div class="box-content">
						<form class="form-horizontal" id="linkform" action="${ctx}/goodslink/update.do?uid=${user.id}&id=${model.id}&gid=${gid}" method="post">
							<fieldset>
							...
							<div class="control-group">
							  ...
							  <div class="controls">
								<textarea  id="goodUrl" name="goodUrl" rows="3">${model.goodUrl}</textarea>
							  </div>
							</div>
							<div class="control-group">
								<label class="control-label" for="focusedInput">价格</label>
								<div class="controls">
								  <input class="input-xlarge focused" id="price" name="price" type="text" value="${model.price}">
								</div>
							 </div>
							 <div class="control-group">
								<label class="control-label" for="focusedInput">排序值</label>
								<div class="controls">
								  <input class="input-xlarge focused" id="origin" name="origin" type="text" value="${model.origin}">
								</div>
							  </div>
							</fieldset>
						  </form>
...
<script type="text/javascript">
	...
	$(function () {
        ...
        $("#confirmbtn").click(function () {
            ...
            if (abstracts == "" || goodUrl == "" || price == "" || origin == "" ) {
                alert("对不起! 您的[摘要]、[链接地址]、[价格]和[排序值]为必填项！");
                return false;
            }else{
            	$("#linkform").submit();
            }
