	...
	<!-- topbar ends -->
		...
		<div class="box span12">
					...
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> 文章分类管理</h2>
						<div class="box-icon">
							<a href="${ctx}/assortarticle/operation.do?uid=${user.id}&id=0&flag=add" class="btn btn-add btn-round"><i class=" icon-plus"></i></a>
						</div>
					</div>
						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
							...
							<tr>
								...
								<td>
									<c:choose>
										<c:when test="${node.parentid==0}">无</c:when>
										<c:when test="${node.parentid > 1}">${node.parentid}</c:when>
									</c:choose>
								</td>
								<td class="center">
									<a class="btn btn-info" href="${ctx}/assortarticle/operation.do?uid=${user.id}&id=${node.id}&flag=update">
										<i class="icon-edit icon-white"></i>  
										修改                                            
									</a>
									<a class="btn btn-danger" href="${ctx}/assortarticle/delete.do?uid=${user.id}&id=${node.id}">
