...
<body>
				...
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-edit"></i> 添加商品专题</h2>
						<div class="box-icon">
							<a href="${ctx}/topicgoods/info.do?uid=${user.id}" class="btn btn-back btn-round"><i class="icon-remove"></i></a>
						</div>
					</div>
					<div class="box-content">
						<form class="form-horizontal" id="topgoodsform" action="${ctx}/topicgoods/insert.do?submitType=2&uid=${user.id}" method="post">
							<fieldset>
							  ...
							  <div class="controls">
								<textarea id="abstracts" name="abstracts" rows="3"></textarea>
								<span class="help-inline">必填项</span>
							  </div>
							</div>
							  <div class="control-group">
								...
								<div class="controls">
								  <c:forEach items="${crowds}" var="node">
								  	<label class="checkbox inline">
								  		<input type="checkbox" id="person" name= "person" value="${node.id}">${node.name}
								  		</label>
								  </c:forEach>
								</div>
							  </div>
	...
	$(function () {
        ...
        $("#savebtn").click(function () {
            ...
            if (title == "" || abstracts == "" || beginTime == "" || endTime == "" || picUrl == "" ) {
            	...
            	$("#topgoodsform").attr("action", $("#path").val()+"/topicgoods/insert.do?submitType=1&uid="+$("#uid").val());
            	$("#topgoodsform").submit();
            }
        });
