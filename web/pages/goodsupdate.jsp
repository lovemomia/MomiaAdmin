...
<body>
				...
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-edit"></i> 修改商品</h2>
						<div class="box-icon">
							<a href="${ctx}/goods/info.do?uid=${user.id}" class="btn btn-back btn-round"><i class="icon-remove"></i></a>
						</div>
					</div>
					<div class="box-content">
						<form class="form-horizontal" id="goodsform" action="${ctx}/goods/update.do?uid=${user.id}&id=${model.id}" method="post">
							<fieldset>
							...
							<div class="control-group">
								<label class="control-label" for="focusedInput">商品图片</label>
								<div class="controls">
							  ...
							  <div class="control-group">
								...
								<div class="controls">
								  <c:forEach items="${assorts}" var="node">
								  	<label class="checkbox inline">
								  		<input type="checkbox" id="assort" name= "assort" value="${node.id}"  <c:if test="${node.flag==1}">checked</c:if>>${node.name}
								  	</label>
								  </c:forEach>
								</div>
							  </div>
