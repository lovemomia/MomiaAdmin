	...
	<!-- topbar ends -->
		...
		<div class="box span12">
					...
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i>意见反馈</h2>
					</div>
					...
					<form >
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
							<c:forEach items="${feedBacks}" var="node">  
							<tr>
								<td><c:out value="${node.id}"></c:out></td>
								<td><c:out value="${fn:substring(node.content,0,20)}"></c:out></td>
								<td><c:out value="${fn:substring(node.email,0,20)}"></c:out></td>
								<td><c:out value="${node.uid}"></c:out></td>
								<td><c:out value="${fn:substring(node.addTime,0,19)}"></c:out></td>
								<td class="center">
									<a class="btn btn-info" href="${ctx}/feedback/operation.do?uid=${user.id}&id=${node.id}">
										<i class="icon-edit icon-white"></i>详情</a>
								</td>
							</tr>
							</c:forEach>
						  </tbody>
