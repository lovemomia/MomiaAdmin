...
<body>
				...
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-edit"></i> 修改商品专题</h2>
						<div class="box-icon">
							<a href="${ctx}/topicgoods/info.do?uid=${user.id}" class="btn btn-back btn-round"><i class="icon-remove"></i></a>
						</div>
					</div>
					<div class="box-content">
						<form class="form-horizontal" id="topgoodsform" action="${ctx}/topicgoods/update.do?submitType=2&uid=${user.id}&id=${model.id}" method="post">
							<fieldset>
							  ...
							  <div class="control-group">
							  <label class="control-label" for="date01">上线时间</label>
							  <div class="controls">
								<input type="text" class="input-xlarge datepicker" id="beginTime"  name="beginTime" value="${fn:substring(model.beginTime,0,19)}" readonly>至
								<input type="text" class="input-xlarge datepicker" id="endTime"  name="endTime"  value="${fn:substring(model.endTime,0,19)}" readonly>
							  </div>
							</div>
							  <!-- <div class="control-group">
							  ...
							  <div class="control-group">
								...
								<br><c:forEach items="${assorts}" var="node">
								  	...
								  	<c:forEach items="${node.listGoods}" var="entity">
								  		<label class="checkbox inline">
							...
							<div class="form-actions">
								<button id="savebtn" name="savebtn" class="btn btn-primary">草     稿</button>
								<button id="confirmbtn" name="confirmbtn" class="btn btn-primary">发     布</button>
...
<script type="text/javascript">
	...
	$(function () {
        ...
        $("#savebtn").click(function () {
            ...
            if (title == "" || abstracts == "" || beginTime == "" || endTime == "" || picUrl == "" ) {
                alert("对不起! 您的[标题]、[摘要]、[上线时间]或[封面]其中一项为空！");
                return false;
            }else{
            	$("#topgoodsform").attr("action", $("#path").val()+"/topicgoods/update.do?submitType=1&uid="+$("#uid").val()+"&id="+$("#id").val());
            	$("#topgoodsform").submit();
            }
        });
