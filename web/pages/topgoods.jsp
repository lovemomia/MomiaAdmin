		...
		<div class="row-fluid">
			...
			<div class="box span12">
					...
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i>专题商品</h2>
						<div class="box-icon">
							<a href="${ctx}/topicgoods/getgoods.do?uid=${user.id}&id=${model.id}" class="btn btn-add btn-round"><i class=" icon-plus"></i></a>
							<!-- <a href="${ctx}/topicgoods/info.do?uid=${user.id}" class="btn btn-close btn-round"><i class="icon-remove"></i></a> -->
						</div>
					</div>
					<div class="box-content">
					<form class="form-horizontal" id="topform" action="${ctx}/topicgoods/info.do?uid=${user.id}" method="post">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
							<c:forEach items="${goodslist}" var="node">  
							<tr>
								...
								<td><c:out value="${node.uid}"></c:out></td>
								<td><c:out value="${fn:substring(node.addTime,0,19)}"></c:out></td>
								<td class="center">
									<a class="btn btn-danger" href="${ctx}/topicgoods/deletegoods.do?uid=${user.id}&id=${model.id}&gid=${node.id}">
					  	...
					  	<div class="form-actions">
							<button class="btn" id="calbtn" name="calbtn" type="submit">返      回</button>
						</div>  
					  </form>    
