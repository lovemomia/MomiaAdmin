	...
	<!-- topbar ends -->
		...
		<div class="box span12">
					...
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> 文章专题管理</h2>
						<div class="box-icon">
							<a href="${ctx}/topicarticle/operation.do?uid=${user.id}&id=0&flag=add" class="btn btn-add btn-round"><i class=" icon-plus"></i></a>
						</div>
					</div>
					...
					<form >
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
							<c:forEach items="${topics}" var="node">  
							<tr>
								...
								<td>
									<c:choose>
										<c:when test="${node.status==1}">草稿</c:when>
										<c:when test="${node.status==2}">已发布未上线</c:when>
										<c:when test="${node.status==3}">已发布并上线</c:when>
										<c:when test="${node.status==4}">已下线</c:when>
									</c:choose>
								</td>
								<td><c:out value="${fn:substring(node.addTime,0,19)}"></c:out></td>
								<td><c:out value="${fn:substring(node.beginTime,0,19)}"></c:out></td>
								<td><c:out value="${fn:substring(node.endTime,0,19)}"></c:out></td>
								<td class="center">
									<a class="btn btn-info" href="${ctx}/topicarticle/operation.do?uid=${user.id}&id=${node.id}&flag=update">
										<i class="icon-edit icon-white"></i>  修改</a>
									<a class="btn btn-danger" href="${ctx}/topicarticle/delete.do?uid=${user.id}&id=${node.id}">
										<i class="icon-trash icon-white"></i> 	删除	</a>
									<a class="btn btn-success" href="${ctx}/topicarticle/oparticles.do?uid=${user.id}&id=${node.id}" >
									<i class="icon-zoom-in icon-white"></i>文章</a>
								</td>
							</tr>
							</c:forEach>
						  </tbody>
